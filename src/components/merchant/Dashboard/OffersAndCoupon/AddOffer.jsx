...
const OfferModal = ({ restaurantId, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create a plain object instead of FormData if your API expects JSON
      const offerData = {
      ...
      const response = await createOfferByRestaurantOwner(
        restaurantId,
        offerData
      );
  ...
  // Update maxDiscount field when type changes
  useEffect(() => {
    ...
    className="fixed inset-0  bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Conditional Max Discount for percentage */}
          {formData.type === "percentage" && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
