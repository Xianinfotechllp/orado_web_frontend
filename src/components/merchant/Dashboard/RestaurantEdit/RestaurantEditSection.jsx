...
const RestaurantEdit = () => {
  ...
  const handleRestaurantsLoad = (loadedRestaurants) => {
    ...
    if (loadedRestaurants.length > 0) {
      // Only set if no restaurant is selected or if the selected one doesn't exist in the new list
      if (
        !selectedRestaurant ||
        !loadedRestaurants.some((r) => r.id === selectedRestaurant.id)
      ) {
        console.log("Setting default restaurant:", loadedRestaurants[0].id);
        handleRestaurantSelect(loadedRestaurants[0], 0);
      }
    }
  };
  ...
  const handleRestaurantSelect = (restaurant, index) => {
    ...
    console.log("Selected restaurant ID:", restaurant);
    ...
    setFormData({
      ...
      address: {
        ...
        pincode: restaurant.address?.zip || "",
      },
  ...
  const scopeFields = {
    basic: ["name", "merchantSearchName", "minOrderAmount"],
    location: ["address", "location"],
    documents: ["fssaiNumber", "gstNumber"],
    media: ["logo", "coverPhoto"],
    payment: ["paymentMethods"],
  };

  const getUpdatedFields = (original, updated, scope = "all") => {
    const changes = {};

    const allowedFields =
      scope === "all" ? Object.keys(updated) : scopeFields[scope] || [];
  ...
  const handleSave = async (scope = "all") => {
    ...
    try {
      ...
      if (scope === "basic") {
        const updatedFields = getUpdatedFields(selectedRestaurant, formData);
        await updateRestaurantBasicInfo(selectedRestaurant.id, updatedFields);
      }

      if (scope === "location") {
        await updateRestaurantLocation(selectedRestaurant.id, {
  ...
  // ✅ Utility to check for file uploads
  const hasAnyFile = (files) =>
    Object.values(files).some((file) =>
      Array.isArray(file) ? file.length > 0 : file
    );
...
const handleSaveImages = async () => {
  try {
    ...
    await updateRestaurantImages(selectedRestaurant.id, formPayload)
  ...
  const BasicInformationSection = () => (
    <div className="space-y-6">
      {/* <RestaurantSlider onRestaurantSelect={() => { }}  /> */}
            ...
            <div className="bg-white p-4 rounded-lg max-w-lg w-full relative">
              ...
              <LocationPicker
                onSelectLocation={(locationDetails) => {
                  ...
                  setShowLocationPicker(false); // close after selecting
                }}
                initialCoordinates={
                  selectedRestaurant?.location?.coordinates || null
                }
              />
            </div>
          </div>
        ...
        return (
          <MediaGallerySection
            ...
            onFilesSelected={handleFilesSelected}
             onSaveGallery={handleSaveImages} 
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div>
              <div>
                ...
                {/* <RestaurantSlider onRestaurantSelect={() => {}} onRestaurantsLoad={() => {}}  /> */}
              </div>
            </div>
