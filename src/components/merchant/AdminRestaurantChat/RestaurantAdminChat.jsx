...
const RestaurantAdminChat = () => {
            ...
            messages: [
              {
                ...
                sender: {
                  ...data.message.sender,
                  modelType:
                    data.message.senderModel ||
                    (data.message.sender.id === restaurantId
                      ? "restaurant"
                      : "admin"),
                },
              },
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col bg-white h-full">
        <div className="sticky top-0 z-10 bg-gray-800 rounded-t-xl text-white p-4 shadow-md">
        ...
        <div className="sticky bottom-0 p-4 bg-white border-t border-gray-200">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-700 focus:border-transparent"
              disabled={loading}
            />
            <button
              type="submit"
              disabled={!newMessage.trim() || loading}
              className="px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              <Send size={20} />
            </button>
          </form>
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col bg-white h-full">
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-50 text-red-500">
          <p>{error}</p>
        </div>
        {/* Show input even with error */}
        <div className="sticky bottom-0 p-4 bg-white border-t border-gray-400">
          <form onSubmit={handleSendMessage} className="flex gap-2">
            <input
              type="text"
              value={newMessage}
              onChange={handleTyping}
              placeholder="Type your message..."
              className="flex-1 px-4 py-2 border border-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-700 focus:border-transparent"
              disabled={!!error}
            />
            <button
              type="submit"
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-full">
      ...
      {/* Messages */}
      <div
        className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#e4e9f2] "
      ...
      >
        {!chat || chat.messages?.length === 0 ? (
          ...
          chat.messages.map((msg) => {
            const isRestaurant =
              msg.senderModel === "restaurant" ||
              msg.sender?.id === restaurantId;
            return (
              <div
                key={msg._id}
                className={`flex ${
                ...
                <div
                  className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg shadow-sm ${
                    isRestaurant
                      ...
                      : "bg-white text-gray-800 rounded-bl-none border border-gray-400"
                  }`}
                >
                  <p className="text-sm">{msg.content}</p>
                  <p
                    className={`text-xs mt-1 ${
                      isRestaurant ? "text-gray-100" : "text-gray-500"
                    }`}
                  >
                    {formatMessageTime(msg)}
                    {!isRestaurant && msg.readBy?.includes(restaurantId) && (
        ...
        {(isAdminTyping || isTyping) && (
          <div className="flex justify-start">
            <div className="max-w-xs px-4 py-2 rounded-lg rounded-bl-none bg-white text-gray-800 border border-gray-400">
      ...
      <div className="sticky bottom-0 p-4 bg-white border-t border-gray-300">
        <form onSubmit={handleSendMessage} className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#0f172a] text-white rounded-lg hover:bg-[#4d5358] focus:outline-none focus:ring-2 focus:ring-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <Send size={20} />
          </button>
        </form>
