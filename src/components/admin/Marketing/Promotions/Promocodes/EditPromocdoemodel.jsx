...
useEffect(() => {
  if (isOpen && promo) {
    // Prefill form with existing promo data
    setFormData({
      ...promo,
      validFrom: new Date(promo.validFrom),
      validTill: new Date(promo.validTill),
      // Ensure these are arrays, even if empty from backend
    ...
    setCustomerOptions(
      customers.map(c => ({
        value: c._id, // Assuming customer ID is _id as per schema reference
        label: `${c.name} (${c.email || c.id})`
      }))
    );
  }
}, [isOpen, promo, merchants, customers]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // You might want to set a form-level error message here
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Description */}
            <div className="col-span-2"> {/* Made description span full width */}
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Description
              </label>
              <textarea
                ...
                rows="3"
                className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="e.g., Get 20% off on all orders this summer!"
              ></textarea>
            </div>
