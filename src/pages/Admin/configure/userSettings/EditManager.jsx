...
import { getManagerById, updateManager } from '../../../../apis/adminApis/mangerApi';
import { toast } from 'react-toastify';

const EditManager = () => {
  const { mangerId} = useParams();
...
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      // Fetch manager data
      const managerRes = await getManagerById(mangerId);
      console.log(managerRes);
      
      if (managerRes.message === "Manager fetched successfully.") {
        setFormData({
          name: managerRes.manager.name || '',
          email: managerRes.manager.email || '',
          phone: managerRes.manager.phone || '',
          password: '', // Don't pre-fill password for security
          role: managerRes.manager.role?._id || '', // Safe access to role._id
          assignedRestaurants: managerRes.manager.assignedRestaurants?.map(r => r._id) || [],
          viewAllCustomers: managerRes.manager.viewAllCustomers || false
        });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const requestBody = {
        ...
        viewAllCustomers: formData.viewAllCustomers
      };
      ...
      await updateManager(mangerId, requestBody);
      ...
      <section>
        ...
        <div className="content-container bg-white rounded-lg shadow-sm">
          <div className="flex items-center p-4 border-b">
            <h3 className="text-xl font-semibold capitalize">Edit Manager</h3>
          </div>
