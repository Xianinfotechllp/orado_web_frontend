...
const fetchGeofences = async () => {
  try {
    ...
    // Transform API data into map-friendly format
    const transformed = data.map((item) => ({
      id: item._id,  // Make sure to use id consistently
      _id: item._id, // Keep original _id if needed
      name: item.regionName,
      description: item.regionDescription,
      points: item.geometry.coordinates[0], // This matches your API response
...
const HandledeleteGeofence = async (index) => {
  try {
    // Get the geofence ID from your state using the index
    const geofenceToDelete = geofences[index];
    
    // Call the API to delete from backend
    await deleteGeofence(geofenceToDelete.id);  // Changed from _id to id
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="relative">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setActiveGeofence(index); // Keep this geofence active when menu is clicked
              // Toggle menu visibility (you'll need to add state for this)
