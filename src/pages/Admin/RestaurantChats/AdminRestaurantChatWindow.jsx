...
const AdminRestaurantChatWindow = ({ restaurantId, chatId }) => {
  ...
  useEffect(() => {
    if (!restaurantId || !adminId) return;
    ...
    const fetchChat = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:5000/chat/admin/restaurants/${restaurantId}`, {
          ...
          messages: data.data.messages.map(msg => ({
            ...
            sender: {
              ...
              modelType: msg.senderModel || 
                        (msg.sender.id === adminId ? 'admin' : 'restaurant')
            }
          }))
        };
            ...
            messages: [...prev.messages, {
              ...
              sender: {
                ...
                modelType: data.message.senderModel || 
                          (data.message.sender.id === adminId ? 'admin' : 'restaurant')
              }
            }]
          };
        });
    ...
    const handleTypingIndicator = (data) => {
      if (data.chatId === chat?._id && data.senderModel === 'restaurant') {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem('adminToken');
      const res = await fetch(`http://localhost:5000/chat/admin/restaurants/${restaurantId}/message`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const restaurant = chat.participants?.find(p => p.modelType === 'restaurant')?.id;

  return (
    <div className="flex-1 flex flex-col bg-white">
      <div className="p-4 bg-orange-500 text-white border-b border-orange-600">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-white bg-opacity-20 rounded-full flex items-center justify-center font-semibold">
            {restaurant?.name?.split(' ').map(n => n[0]).join('') || 'R'}
          </div>
          <div>
            <h2 className="text-lg font-semibold">{restaurant?.name || 'Restaurant'}</h2>
...
export default AdminRestaurantChatWindow;