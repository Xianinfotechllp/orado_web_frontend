...
const AdminRestaurantChatList = ({ onSelectChat, selectedRestaurantId }) => {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        ...
        const response = await fetch('http://13.60.170.28:5000/chat/admin/restaurants', {
          headers: {
        ...
        data.data.forEach(chat => {
          const restaurant = chat.participants.find(p => p.modelType === 'restaurant')?.id?._id;
          if (restaurant && !seenRestaurantIds.has(restaurant)) {
            seenRestaurantIds.add(restaurant);
            uniqueChats.push({
              ...chat,
              unreadCount: 0
            });
    ...
    const handleNewMessage = (data) => {
      setChats(prevChats => {
        ...
        if (chatIndex !== -1) {
          ...
          const senderIsRestaurant = data.message.sender.modelType === 'restaurant';
          const updatedChat = {
            ...chat,
            lastMessage: data.message,
            updatedAt: new Date(),
            unreadCount: senderIsRestaurant ? (chat.unreadCount || 0) + 1 : 0
          };
  ...
  const filteredChats = chats.filter(chat => {
    ...
    const email = typeof restaurant?.email === 'string' ? restaurant.email.toLowerCase() : '';
        ...
        {filteredChats.length === 0 ? (
          ...
          filteredChats.map((chat) => {
            ...
            return (
              <div
                key={chat._id}
                onClick={() => onSelectChat(restaurant?._id, chat._id)}
...
export default AdminRestaurantChatList;