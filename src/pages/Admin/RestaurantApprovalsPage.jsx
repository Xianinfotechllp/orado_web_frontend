...
import {
  FiEye, FiCheck, FiX, FiSearch, FiFilter, FiChevronDown,
  FiClock, FiAlignLeft, FiMail, FiPhone, FiMessageSquare,
  FiBarChart2, FiPieChart, FiTrendingUp,FiInfo,FiUser,FiMapPin,FiImage
} from 'react-icons/fi';
...
const RestaurantApprovalsPage = () => {
  ...
  // Analytics data
  const statusData = [
    { name: 'Approved', value: restaurants.filter(r => r.status === 'Approved').length },
    { name: 'Pending', value: restaurants.filter(r => r.status === 'Pending').length },
    { name: 'Rejected', value: restaurants.filter(r => r.status === 'Rejected').length },
  ];
      ...
      try {
        ...
        const formatted = response.data.restaurants.map((restaurant) => ({
          ...
          openingHours: restaurant.openingHours ?
            ...
            'Not specified',
          paymentMethods: restaurant.paymentMethods ? restaurant.paymentMethods.join(', ') : 'Not specified',
  ...
  const sendMessage = async () => {
    ...
    try {
      console.log(`Sending ${contactMethod} to ${selectedRestaurant.email}: ${message}`);
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`${activeTab === 'approvals' ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}
          >
            Approval Requests
          </button>
          <button
            onClick={() => setActiveTab('analytics')}
            className={`${activeTab === 'analytics' ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}
          >
            Analytics Dashboard
          </button>
        </nav>
      ...
      {activeTab === 'approvals' ? (
        <>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Restaurant Approvals</h1>
          ...
          <div className="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200">
            {loading ? (
              <div className="p-8 text-center">
                ...
                <p className="mt-4 text-gray-600">Loading restaurant applications...</p>
              </div>
            ) : (
                    ...
                    {filtered.map((r) => (
                      <tr key={r.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm text-gray-500">{r.merchantSearchName}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          {r.kycStatus === 'rejected' && (
                            <div className="text-xs text-red-500 mt-1">{r.kycRejectionReason}</div>
                          )}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {r.createdAt}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            ...
                            <button
                              ...
                              className={`p-1 rounded-full ${r.status === 'Approved' ? 'text-gray-400 cursor-not-allowed' : 'text-green-600 hover:text-green-900 hover:bg-green-50'}`}
                              title="Approve"
                            >
                            ...
                            <button
                              ...
                              className={`p-1 rounded-full ${r.status === 'Rejected' ? 'text-gray-400 cursor-not-allowed' : 'text-red-600 hover:text-red-900 hover:bg-red-50'}`}
                              title="Reject"
                            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-green-50 p-4 rounded-lg border border-green-100">
              ...
              <p className="text-xs text-green-600 mt-1">
                {restaurants.length > 0 ?
                  `${Math.round((restaurants.filter(r => r.status === 'Approved').length / restaurants.length) * 100)}% of total` :
                  'No data'}
              </p>
            </div>
            ...
            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100">
              ...
              <p className="text-xs text-yellow-600 mt-1">
                {restaurants.length > 0 ?
                  `${Math.round((restaurants.filter(r => r.status === 'Pending').length / restaurants.length) * 100)}% of total` :
                  'No data'}
              </p>
            </div>
            ...
            <div className="bg-red-50 p-4 rounded-lg border border-red-100">
              ...
              <p className="text-xs text-red-600 mt-1">
                {restaurants.length > 0 ?
                  `${Math.round((restaurants.filter(r => r.status === 'Rejected').length / restaurants.length) * 100)}% of total` :
                  'No data'}
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex items-center mt-2 space-x-4">
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-semibold text-gray-700">{selectedRestaurant.rating}/5</span>
                </div>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                  ...
                  {selectedRestaurant.active === 'Yes' ? 'Active' : 'Inactive'}
                </span>
              </div>
            </div>
            <button
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-bold text-orange-600">₹{selectedRestaurant.minOrderAmount}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-sm text-red-700 mt-1">{selectedRestaurant.kycRejectionReason}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm text-gray-900 font-medium">{selectedRestaurant.foodType}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Payment Methods:</span>
                        <span className="text-sm text-gray-900">{selectedRestaurant.paymentMethods}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Opening Hours:</span>
                        <span className="text-sm text-gray-900">{selectedRestaurant.openingHours}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-500">Owner ID: </span>
                        <span className="text-sm text-gray-900 ml-1">{selectedRestaurant.ownerId}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
                    ...
                    <p className="text-sm text-gray-700 leading-relaxed">{selectedRestaurant.address}</p>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
                    ...
                    {selectedRestaurant.banners.length > 0 && (
                      <div>
                        <h5 className="text-sm font-medium text-gray-600 mb-2">Promotional Banners</h5>
                        <div className="grid grid-cols-2 gap-2">
                          {selectedRestaurant.banners.slice(0, 4).map((banner, index) => (
                            <img 
                              key={index} 
                              src={banner} 
                              alt={`Banner ${index + 1}`} 
                        ...
                        {selectedRestaurant.banners.length > 4 && (
                          <p className="text-xs text-gray-500 mt-1">+{selectedRestaurant.banners.length - 4} more banners</p>
                        )}
                      </div>
                    )}
                    
                    {selectedRestaurant.images.length === 0 && selectedRestaurant.banners.length === 0 && (
              ...
              <div className="mt-8 bg-gradient-to-r from-orange-50 to-orange-100 rounded-lg p-6 border border-orange-200">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    <FiMail className="mr-2 h-4 w-4" /> Send Email
                  </button>
                  <button
                    onClick={() => setContactMethod('call')}
                    className="flex items-center justify-center px-6 py-3 bg-white border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200"
                  >
                    <FiPhone className="mr-2 h-4 w-4" /> Make Call
                  </button>
                  <button
                    onClick={() => setContactMethod('sms')}
                    className="flex items-center justify-center px-6 py-3 bg-white border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200"
                  >
                    <FiMessageSquare className="mr-2 h-4 w-4" /> Send SMS
                  </button>
                </div>
              </div>
            </>
            ...
            <div className="max-w-2xl mx-auto">
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-gray-600">
                  Compose your message below and it will be sent via {contactMethod}
                </p>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <span className="text-xs text-gray-500">
                    To: {contactMethod === 'email' ? selectedRestaurant.email : selectedRestaurant.phone}
                  </span>
                </div>
              </div>
            </div>
          ...
          {contactMethod ? (
            <>
              ...
              >
                ...
                Send {contactMethod.charAt(0).toUpperCase() + contactMethod.slice(1)}
              </button>
            </>
          ) : (
            <>
              ...
              >
                ...
                {selectedRestaurant.status === 'Rejected' ? 'Already Rejected' : 'Reject Application'}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {selectedRestaurant.status === 'Approved' ? 'Already Approved' : 'Approve Application'}
              </button>
            </>
          )}
        </div>
