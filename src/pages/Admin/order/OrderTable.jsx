  ...
  const OrdersTable = () => {
    ...
    'arrived',
    'delivered',
    'cancelled_by_customer'  // if you still want admin to cancel — or remove if not
  ];
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      // Call your API here to update status
      await updateOrderStatus(orderId, newStatus);  // assuming this is your API function
  ...
  <span>{order.restaurantName}</span>
                            ...
                            {order.customerId ? (
  <Link to={`/admin/dashboard/customer/${order.customerId}/details`} className="font-medium text-blue-600 hover:underline">
    {order.customerName}
  </Link>
) : (
